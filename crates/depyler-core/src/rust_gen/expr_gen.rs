            ...
            let borrowed_args: Vec<syn::Expr> = hir_args
                ...
                .map(|(param_idx, (hir_arg, arg_expr))| {
                    ...
                    let (should_borrow, is_mutable) = match hir_arg {
                        HirExpr::Var(var_name) => {
                            // Check if variable has List, Dict, Set, String, or class type
                            if let Some(var_type) = self.ctx.var_types.get(var_name) {
                                if matches!(
                                    var_type,
                                    ...
                                    let should_borrow = self.ctx
                                        .function_param_borrows
                                        .get(func)
                                        .and_then(|borrows| borrows.get(param_idx))
                                        .map(|(borrowed, _)| *borrowed)
                                        .unwrap_or(true); // Default to borrow (&str) if unknown
                                    (should_borrow, false) // Strings are never mutably borrowed in this context
                                } else {
                                    (false, false)
                                }
                            } else {
                        ...
                        HirExpr::Literal(crate::hir::Literal::String(_)) => {
                            ...
                            (false, false) // Don't add & reference for strings
                        }
                        ...
                        HirExpr::Binary {
                            ...
                            if is_string_concat {
                                ...
                                let should_borrow = self.ctx
                                    ...
                                    .map(|(borrowed, _)| *borrowed)
                                    .unwrap_or(true); // Default to borrow (&str) if unknown
                                (should_borrow, false) // String borrows are never mutable
                            } else {
                                (false, false)
                            }
                        }
