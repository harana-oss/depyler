...
//!
//! Provides [`TypeVarRegistry`] and [`GenericType`] for inferring generic type
//! parameters from Python function signatures and bodies.
...
impl TypeVarRegistry {
    ...
    pub fn to_rust_generatoreric(&self, name: &str, params: &[Type]) -> String {
        if params.is_empty() {
            name.to_string()
        } else {
    ...
    fn check_binary_op_usage(
        ...
        if uses_param {
            let constraint = match op {
                ...
                BinOp::Eq | BinOp::NotEq | BinOp::Is | BinOp::IsNot => {
