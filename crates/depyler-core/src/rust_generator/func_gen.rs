...
use crate::hir::*;
use crate::rust_generator::context::{CodeGenContext, RustCodeGen};
use crate::rust_generator::generator_gen::codegen_generator_function;
use crate::rust_generator::type_gen::{rust_type_to_syn, update_import_needs};
use anyhow::Result;
use quote::quote;
use std::collections::{HashMap, HashSet};
use syn::{self, parse_quote};

// Import analyze_mutable_vars from codegen module (moved in §31 refactor)
use super::codegen::analyze_mutable_vars;
...
pub(crate) fn codegen_generic_params(
    type_params: &[crate::types::type_inference::TypeParameter],
...
pub(crate) fn codegen_function_body(
    func: &HirFunction,
    can_fail: bool,
    error_type: Option<crate::rust_generator::context::ErrorType>,
...
pub(crate) fn codegen_function_params(
    func: &HirFunction,
    lifetime_result: &crate::analysis::lifetime_analysis::LifetimeResult,
    ctx: &mut CodeGenContext,
) -> Result<Vec<proc_macro2::TokenStream>> {
    func.params
        .iter()
...
fn codegen_single_param(
    ...
    let takes_ownership = matches!(
        ...
        Some(crate::analysis::borrowing_context::BorrowingStrategy::TakeOwnership) | None
    );
    ...
    if let Some(inferred) = lifetime_result.param_lifetimes.get(&param.name) {
        ...
        let actual_rust_type =
            if let crate::types::type_mapper::RustType::Enum { name, variants: _ } = rust_type {
                if name == "UnionType" {
                    if let Type::Union(types) = &param.ty {
                        let enum_name = ctx.process_union_type(types);
                        crate::types::type_mapper::RustType::Custom(enum_name)
                    } else {
                        rust_type.clone()
                    }
                } else {
...
fn apply_param_borrowing_strategy(
    ...
    inferred: &crate::analysis::lifetime_analysis::InferredParam,
    lifetime_result: &crate::analysis::lifetime_analysis::LifetimeResult,
    ctx: &mut CodeGenContext,
) -> Result<syn::Type> {
    ...
    if let Some(strategy) = lifetime_result.borrowing_strategies.get(param_name) {
        match strategy {
            crate::analysis::borrowing_context::BorrowingStrategy::UseCow { lifetime } => {
...
fn apply_borrowing_to_type(
    ...
    if matches!(rust_type, crate::types::type_mapper::RustType::String) {
...
) -> crate::types::type_mapper::RustType {
...
fn apply_indexed_field_return_lifetimes(
    func: &HirFunction,
    lifetime_result: &mut crate::analysis::lifetime_analysis::LifetimeResult,
    ref_var_sources: &HashMap<String, String>,
) {
...
pub(crate) fn codegen_return_type(
    ...
    crate::types::type_mapper::RustType,
    bool,
    Option<crate::rust_generator::context::ErrorType>,
    Type,
)> {
    ...
    let rust_ret_type =
        if let crate::types::type_mapper::RustType::Enum { name, .. } = &mapped_ret_type {
            if name == "UnionType" {
    ...
    let rust_ret_type =
        if matches!(func.ret_type, Type::String) && function_returns_owned_string(func) {
            ...
            crate::types::type_mapper::RustType::String
        } else {
            rust_ret_type
        };
    ...
    let rust_ret_type = if !ctx.return_reference_positions.is_empty() {
        if let crate::types::type_mapper::RustType::Tuple(ref elements) = rust_ret_type {
            if ctx.return_reference_positions.len() == elements.len() {
                let new_elements: Vec<crate::types::type_mapper::RustType> = elements
                    .iter()
                    ...
                    .collect();
                crate::types::type_mapper::RustType::Tuple(new_elements)
            } else {
                rust_ret_type
            }
        } else {
    ...
    let can_fail = false;
    let error_type: Option<crate::rust_generator::context::ErrorType> = None;

    let return_type = if matches!(rust_ret_type, crate::types::type_mapper::RustType::Unit) {
        quote! {}
    } else {
        ...
        if !returns_concatenation {
            ...
            for param in &func.params {
                if let Some(strategy) = lifetime_result.borrowing_strategies.get(&param.name) {
                    if matches!(
                        strategy,
                        crate::analysis::borrowing_context::BorrowingStrategy::UseCow { .. }
                    ) {
        ...
        if uses_cow_return && !returns_concatenation {
            ...
            if let Some(ref return_lt) = lifetime_result.return_lifetime {
                ...
                if matches!(
                    rust_ret_type,
                    crate::types::type_mapper::RustType::Str { .. }
                        | crate::types::type_mapper::RustType::Reference { .. }
...
impl RustCodeGen for HirFunction {
    fn to_rust_tokens(&self, ctx: &mut CodeGenContext) -> Result<proc_macro2::TokenStream> {
        ...
        // NOTE: function_param_borrows is now pre-populated in rust_generator.rs::populate_function_param_borrows()
        ...
        let subcommand_info = if ctx.argparser_tracker.has_subcommands() {
            crate::rust_generator::argparse_transform::analyze_subcommand_field_access(
                self,
        ...
        if ctx.argparser_tracker.has_parsers() {
            if ctx.argparser_tracker.get_first_parser().is_some() {
                ctx.require(crate::rust_generator::context::Import::Clap);

                let commands_enum =
                    crate::rust_generator::argparse_transform::generate_commands_enum(
                        &ctx.argparser_tracker,
                    );
                ...
                let parser_info = ctx.argparser_tracker.get_first_parser().unwrap();
                let args_struct = crate::rust_generator::argparse_transform::generate_args_struct(
                    parser_info,
                    &ctx.argparser_tracker,
                );
                ctx.generated_args_struct = Some(args_struct);

                // Note: ArgumentParser-related statements are filtered in statements.rs
                // parse_args() calls are transformed in statements.rs::codegen_assign_stmt
            }
        ...
        if let Some((variant_name, fields)) = subcommand_info {
            ...
            if let Some(args_param) = self.params.first() {
                ...
                body_stmts =
                    crate::rust_generator::argparse_transform::wrap_body_with_subcommand_pattern(
                        body_stmts,
                        &variant_name,
                        &fields,
                        args_param_name,
