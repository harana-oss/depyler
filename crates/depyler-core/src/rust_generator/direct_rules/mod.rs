...
pub use class_gen::{convert_class_to_struct, method_mutates_self};
pub use enum_gen::{convert_class_to_enum, convert_class_to_intflag, to_pascal_case};
pub(crate) use trait_gen::convert_abc_to_trait;

use crate::hir::*;
use crate::types::type_mapper::{RustType, TypeMapper};
...
fn convert_stmt_with_context(
    ...
    match stmt {
        ...
        HirStmt::FunctionDef { .. } => {
            // Nested functions are handled by the main rust_generator module
        ...
        HirStmt::AsyncFunctionDef { .. } => {
            // Async nested functions handled by the main rust_generator module
            Ok(syn::Stmt::Expr(parse_quote! { {} }, None))
        }
...
impl<'a> ExprConverter<'a> {
    fn with_classmethod(_type_mapper: &'a TypeMapper, is_classmethod: bool) -> Self {
        Self {
            is_classmethod,
            field_types: None,
            current_trait: None,
    ...
    fn convert_index(&self, base: &HirExpr, index: &HirExpr) -> Result<syn::Expr> {
        ...
        // Direct indexing for simplicity (matches Python behavior)
        Ok(parse_quote! {
