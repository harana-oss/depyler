...
fn handle_max_two(args: &[HirExpr], ctx: &mut CodeGenContext) -> Result<syn::Expr> {
    ...
    let arg1_is_float = ctx.expr_has_type(&args[0], &Type::Float);
    let arg2_is_float = ctx.expr_has_type(&args[1], &Type::Float);
...
fn handle_max_multiple(args: &[HirExpr], ctx: &mut CodeGenContext) -> Result<syn::Expr> {
    ...
    let is_float = args.iter().any(|arg| ctx.expr_has_type(arg, &Type::Float));

    if is_float {
        // Chain f64::max calls: f64::max(f64::max(a, b), c)
        // Cast non-float arguments to f64
        let first_is_float = ctx.expr_has_type(&args[0], &Type::Float);
        ...
        for arg in &args[1..] {
            let arg_is_float = ctx.expr_has_type(arg, &Type::Float);
