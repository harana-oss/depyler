...
pub fn handle_round(args: &[HirExpr], ctx: &mut CodeGenContext) -> Result<syn::Expr> {
    ...
    // Handle round(x, ndigits) - round to specific decimal places
    if args.len() == 2 {
        let ndigits_expr = args[1].to_rust_expr(ctx)?;

        // Python's round(x, n) rounds to n decimal places
        // In Rust: (x * 10^n).round() / 10^n
        if needs_float_cast {
            Ok(parse_quote! {
                {
                    let multiplier = (10.0_f64).powi(#ndigits_expr as i32);
                    ((#value_expr as f64) * multiplier).round() / multiplier
                }
            })
        } else {
            Ok(parse_quote! {
                {
                    let multiplier = (10.0_f64).powi(#ndigits_expr as i32);
                    (#value_expr * multiplier).round() / multiplier
                }
            })
        }
