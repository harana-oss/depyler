...
use crate::rust_generator::func_gen::{infer_expr_type_with_env, infer_iter_element_type};
            ...
            if method == "ArgumentParser" {
                if let HirExpr::Var(module_name) = object.as_ref() {
                    if module_name == "argparse" {
                        ...
                        let mut info = crate::rust_generator::argparse_transform::ArgParserInfo::new(
                            var_name.clone(),
                        );
                ...
                if let HirExpr::Var(parser_var) = object.as_ref() {
                    if ctx.argparser_tracker.get_parser(parser_var).is_some() {
                        ...
                        if let AssignTarget::Symbol(subparsers_var) = target {
                            use crate::rust_generator::argparse_transform::SubparserInfo;
                        ...
                        .is_some()
                    {
                        ...
                        if !args.is_empty() {
                            ...
                            if let AssignTarget::Symbol(subcommand_var) = target {
                                use crate::rust_generator::argparse_transform::SubcommandInfo;
