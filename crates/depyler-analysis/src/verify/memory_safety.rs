//! Memory safety analysis for HIR functions

use super::{PropertyStatus, TestCase, VerificationMethod, VerificationResult};
...
impl MemorySafetyAnalyzer {
    ...
    pub fn analyze_function(&mut self, func: &HirFunction) -> VerificationResult {
        ...
        if violations.is_empty() {
            ...
            let violation_messages: Vec<String> = violations.iter().map(|v| format!("{v:?}")).collect();
    ...
    fn check_binary_moves(&self, left: &HirExpr, right: &HirExpr, location: &str) -> Option<MemorySafetyViolation> {
    ...
    fn check_index_moves(&self, base: &HirExpr, index: &HirExpr, location: &str) -> Option<MemorySafetyViolation> {
