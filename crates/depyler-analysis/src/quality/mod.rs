//! Quality analysis and gates
//!
//! Provides quality gates, PMAT metrics, and code quality analysis.

use crate::metrics::{calculate_cognitive, calculate_cyclomatic, count_statements};
...
impl QualityAnalyzer {
    ...
    fn calculate_complexity_metrics(&self, functions: &[HirFunction]) -> ComplexityMetrics {
        ...
        let max_nesting = functions
            .iter()
            .map(|f| crate::metrics::calculate_max_nesting(&f.body))
            .max()
            .unwrap_or(0);
