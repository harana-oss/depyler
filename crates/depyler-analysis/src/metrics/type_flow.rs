//! Type flow analysis and inference
//!
//! Provides type inference for Python code that lacks explicit type annotations.
...
impl TypeInferencer {
    ...
    fn infer_tuple(&mut self, elts: &[HirExpr]) -> Result<Type> {
        let types: Vec<Type> = elts.iter().map(|e| self.infer_expr(e)).collect::<Result<Vec<_>>>()?;
        Ok(Type::Tuple(types))
    }
