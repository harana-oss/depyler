...
def add_conversion(state: State) -> None:
    print(">>>>>>>> [Conversion] Adding conversion points to state")
...
def convert_field_position(state: State, coordinate: str, team: str) -> FieldPosition:
    y_position = KICKOFF_RESULTS.index(coordinate) % 6
    x_position = KICKOFF_RESULTS.index(coordinate) // 6
...
def _create_player_statistics(player: Player, include_breakdown: bool) -> PlayerStatistics:
    period_breakdown = [_create_statistics() for _ in range(NUM_PERIODS)] if include_breakdown else []
    ...
    for team in ("Home", "Away"):
        ...
        for player in players:
            player.period_statistics = [
                _create_player_statistics(player, include_breakdown=False) for _ in range(NUM_PERIODS)
            ]
            total_statistics = _create_player_statistics(player, include_breakdown=True)
...
def nrl(state: State) -> None:
    ...
    while True:
        ...
        if state.include_players and minute_delta > 0:
            ...
            apply_time_on_ground(state, team="Home", seconds=float(minute_delta * 60))
...
def check_game_status(state: State) -> None:
    ...
    if state.time_elapsed > HALF_LENGTH_IN_SECONDS and state.period.name != 'ExtraTime':
        ...
        if state.period.name == 'FirstHalf':
            ...
            update_state[State](lambda s: setattr(s.period, 'number', 2))
            update_state[State](lambda s: setattr(s.period, 'name', "SecondHalf"))
            update_state[State](lambda s: setattr(s, 'game_status', "Period2"))
        else:
            ...
            if state.time_elapsed >= GAME_LENGTH_IN_SECONDS:
                ...
                if state.home_match_score == state.away_match_score:
                    ...
                    # Enter extra time
                    update_state[State](lambda s: setattr(s, 'is_extratime', True))
                    update_state[State](lambda s: setattr(s, 'game_status', "ExtraTime"))
                    update_state[State](lambda s: setattr(s.period, 'name', "ExtraTime"))
                    update_state[State](lambda s: setattr(s.period, 'number', 3))
                else:
                    print("[CheckGameStatus] End game")
                    # End game
                    update_state[State](lambda s: setattr(s, 'game_status', "Ended"))
                    update_state[State](lambda s: setattr(s, 'is_over', True))
...
def clock(state: State) -> ClockOutputs:
    ...
    # Update game state
    update_state[State](lambda s: setattr(s, 'time_elapsed', state.time_elapsed + int(seconds_to_add)))
...
def get_kickoff_model_result(state: State) -> GetKickoffModelResultOutputs:
    ...
    return GetKickoffModelResultOutputs(
        ...
        grid_result=KICKOFF_TO_GRID_RESULT[kickoff_result]
    )
...
def register_events() -> None:
    ...
    register_event_with_when("debug", lambda state: state.is_over, debug)
    register_event_with_when("anytime_output_event", lambda state: False, anytime_output_event)
    register_event_with_when("end_of_game_output_event", lambda state: state.game_status == 'ENDED', end_of_game_output_event)
    register_event_with_when("first_half_output_event", lambda state: False, first_half_output_event)
    register_event_with_when("normal_time_output_event", lambda state: False, normal_time_output_event)
    register_event_with_when("player_void_output_event", lambda state: True, player_void_output_event)
    register_event_with_when("period_first_last_score_helper", lambda state: False, period_first_last_score_helper)
    register_event_with_when("first_to_score_tracking_event", lambda state: len(state.incidents) > 0 or state.is_over, first_to_score_tracking_event)
    register_event_with_when("minute_winner_tracking_event", lambda state: len(state.incidents) > 0 or state.is_over, minute_winner_tracking_event)
    register_event_with_when("player_score_tracking_event", lambda state: len(state.incidents) > 0, player_score_tracking_event)
    register_event_with_when("race_to_points_tracking_event", lambda state: len(state.incidents) > 0 or state.is_over, race_to_points_tracking_event)
