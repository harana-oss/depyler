...
fn get_depyler_bin() -> Option<&'static str> {
    option_env!("CARGO_BIN_EXE_depyler")
}

#[test]
fn test_full_lambda_workflow() -> Result<()> {
    let Some(depyler_bin) = get_depyler_bin() else {
        eprintln!("Skipping test: depyler binary not available (CARGO_BIN_EXE_depyler not set)");
        return Ok(());
    };
...
"#;
    ...
    let analyze_output = Command::new(depyler_bin)
    ...
    let convert_output = Command::new(depyler_bin)
        .args([
            "lambda",
            "convert",
...
fn test_lambda_commands_help() -> Result<()> {
    ...
    let help_output = Command::new(depyler_bin).args(["lambda", "--help"]).output()?;
