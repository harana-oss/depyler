...
"#;
    ...
    let rust_code = result.unwrap();
    assert!(rust_code.contains("state: &State"));
    // Should ideally iterate with &state.items for read-only access
}
...
"#;
    ...
    let rust_code = result.unwrap();
    assert!(rust_code.contains("state: &State"));
    // Should use .iter().enumerate() in Rust
}
...
"#;
    ...
    let rust_code = result.unwrap();
    assert!(rust_code.contains("state: &mut State"));
    // Should ideally use .iter_mut() in Rust for mutating items
}
...
"#;
    ...
    let rust_code = result.unwrap();
    assert!(rust_code.contains("state: &State"));
    // temp should not be mutable since it's only assigned once
    assert!(!rust_code.contains("let mut temp"));
}
