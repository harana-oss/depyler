...
class Product:
    ...
    def create_sample():
        ...
        return Product("Sample", 9.99, 100)

    @classmethod
    def from_price_only(cls, price: float):
        """Create product with default values"""
        return cls("Unnamed", price, 0)
...
def test_product():
    """Test Product dataclass with static and class methods"""
    # Regular instantiation
    prod = Product("Widget", 19.99, 50)
    assert prod.name == "Widget"
    assert prod.price == 19.99
    assert prod.stock == 50
    assert prod.total_value() == 999.50
    assert prod.is_available()

    # Static method
    sample = Product.create_sample()
    assert sample.name == "Sample"
    assert sample.price == 9.99
    assert sample.stock == 100

    # Class method
    custom = Product.from_price_only(29.99)
    assert custom.name == "Unnamed"
    assert custom.price == 29.99
    assert custom.stock == 0
    assert not custom.is_available()
