...
fn bench_ast_to_hir_conversion(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 200].iter() {
        let source = generate_python_source(*size);
        let ast = rustpython_parser::parse_program(&source).expect("Parse should succeed");

        group.throughput(Throughput::Elements(*size as u64));

        group.bench_with_input(BenchmarkId::new("ast_bridge", size), &ast, |b, ast| {
            b.iter(|| {
...
fn bench_type_inference(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        let hir = pipeline.parse_to_hir(&source).expect("Should parse to HIR");
        ...
        group.bench_with_input(
            ...
            |b, hir| {
                b.iter(|| {
                    ...
                    let mut inferencer = depyler_analysis::TypeInferencer::new();
                    for func in &hir.functions {
                        let _ = inferencer.infer_function(black_box(func));
                    }
                });
            },
        );
...
fn bench_rust_codegen(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        let analyzed_hir = pipeline
            .analyze_to_typed_hir(&source)
