...
# From: event_test-once.py
# @depyler: custom_attribute_ignore = "event"
def test_once_event(state: State) -> None:
    if state.home_match_score > 0:
        print("[Test Once Event] Log once")
        # Log once
        print("This should only appear once when home team scores")
...
def first_to_score_tracking_event(state: State) -> None:
    ...
    if len(state.incidents) > 0 or state.is_over:
        ...
        print("[First To Score Tracking Event] Record match first scores")
        ...
        print("[First To Score Tracking Event] Record Half 2 and Extra Time first scores")
        ...
        print("[First To Score Tracking Event] Record Extra Time first scores")
        ...
        print("[First To Score Tracking Event] Record Half 2 and Extra Time try timings")
        ...
        print("[First To Score Tracking Event] Record Extra Time try timings")
        ...
        print("[First To Score Tracking Event] Record match first try timings")
        ...
        print("[First To Score Tracking Event] Record last scoring outputs when match ended")
            ...
            if state.period.name == 'FirstHalf':
                ...
                if state.time_elapsed >= GAME_LENGTH_IN_SECONDS:
                    ...
                    if state.home_match_score == state.away_match_score:
                        ...
                        print("[Update Game Status] End game")
                        # End game
                        state.game_status = "Ended"
                        state.is_over = True

        print("[Update Game Status] Check extra time conditions")
        # Check extra time conditions
        if state.period.name == 'ExtraTime':
            print("[Update Game Status] Check if extra time should end")
...
def convert_field_position(state: State, grid_result: str, team: str) -> FieldPosition:
...
def nrl(state: State) -> None:
    ...
    while True:
        ...
        # Evaluate field goal
        field_goal_decision = field_goal_decision(state)
        ...
        if field_goal_decision.attempt:
            ...
            field_goal_attempt_outcome = field_goal_attempt(state)
        ...
        if field_goal_attempt_result == 1:
            ...
            # Get next play type
            next_play_result = next_play(state)
...
# From: step_add_conversion.py
# @depyler: custom_attribute_ignore = "step"
def add_conversion(state: State) -> None:
    print("[AddConversion] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[AddConversion] Add conversion points to home team")
    # Add conversion points to home team
    if state.team_in_possession == "Home":
        print("[AddConversion] Set variables")
        # Set variables
    ...
    print("[AddConversion] Add conversion points to away team")
    # Add conversion points to away team
    if state.team_in_possession == "Away":
        print("[AddConversion] Set variables")
        # Set variables
    ...
    print("[AddConversion] Update home team statistics")
    # Update home team statistics
    if state.team_in_possession == "Home":
        print("[AddConversion] Update home period conversions")
        # Update home period conversions
        state.home_statistics.period_statistics[period_idx].scores.total = state.home_statistics.period_statistics[period_idx].scores.total + CONVERSION_POINTS
        state.home_statistics.period_statistics[period_idx].scores.conversions = state.home_statistics.period_statistics[period_idx].scores.conversions + 1

        execute_events(state)

        print("[AddConversion] Update home total conversions")
        # Update home total conversions
        state.home_statistics.total_statistics.scores.total = state.home_statistics.total_statistics.scores.total + CONVERSION_POINTS
        state.home_statistics.total_statistics.scores.conversions = state.home_statistics.total_statistics.scores.conversions + 1

        execute_events(state)

    execute_events(state)

    print("[AddConversion] Update away team statistics")
    # Update away team statistics
    if state.team_in_possession == "Away":
        print("[AddConversion] Update away period conversions")
        # Update away period conversions
        state.away_statistics.period_statistics[period_idx].scores.conversions = state.away_statistics.period_statistics[period_idx].scores.conversions + 1
        state.away_statistics.period_statistics[period_idx].scores.total = state.away_statistics.period_statistics[period_idx].scores.total + CONVERSION_POINTS

        execute_events(state)

        print("[AddConversion] Update away total conversions")
        # Update away total conversions
        state.away_statistics.total_statistics.scores.conversions = state.away_statistics.total_statistics.scores.conversions + 1
        state.away_statistics.total_statistics.scores.total = state.away_statistics.total_statistics.scores.total + CONVERSION_POINTS

        execute_events(state)

    execute_events(state)

    print("[AddConversion] Update player statistics if enabled")
    # Update player statistics if enabled
    if state.include_players:
        print("[AddConversion] Update home player statistics")
        # Update home player statistics
        if state.team_in_possession == "Home":
            print("[AddConversion] Set variables")
            # Set variables
            state.home_player_selected_for_points_market.total_statistics.scores.conversions = state.home_player_selected_for_points_market.total_statistics.scores.conversions + 1
            state.home_player_selected_for_points_market.period_statistics[period_idx].scores.conversions = state.home_player_selected_for_points_market.period_statistics[period_idx].scores.conversions + 1
            state.home_player_selected_for_points_market.total_statistics.scores.total = state.home_player_selected_for_points_market.total_statistics.scores.total + CONVERSION_POINTS
            state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total + CONVERSION_POINTS

            execute_events(state)

        execute_events(state)

        print("[AddConversion] Update away player statistics")
        # Update away player statistics
        if state.team_in_possession == "Away":
            print("[AddConversion] Set variables")
            # Set variables
            state.away_player_selected_for_points_market.total_statistics.scores.total = state.away_player_selected_for_points_market.total_statistics.scores.total + CONVERSION_POINTS
            state.away_player_selected_for_points_market.period_statistics[period_idx].scores.conversions = state.away_player_selected_for_points_market.period_statistics[period_idx].scores.conversions + 1
            state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total + CONVERSION_POINTS
            state.away_player_selected_for_points_market.total_statistics.scores.conversions = state.away_player_selected_for_points_market.total_statistics.scores.conversions + 1
...
# From: step_add_field_goal.py
# @depyler: custom_attribute_ignore = "step"
def add_field_goal(state: State) -> None:
    print("[AddFieldGoal] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[AddFieldGoal] Calculate points")
    # Calculate points
    if is_two_pointer:
        print("[AddFieldGoal] Set variables")
        # Set variables
    ...
    print("[AddFieldGoal] Add field goal points to home team")
    # Add field goal points to home team
    if state.team_in_possession == "Home":
        print("[AddFieldGoal] Set variables")
        # Set variables
    ...
    print("[AddFieldGoal] Add field goal points to away team")
    # Add field goal points to away team
    if state.team_in_possession == "Away":
        print("[AddFieldGoal] Set variables")
        # Set variables
    ...
    print("[AddFieldGoal] Update home team statistics")
    # Update home team statistics
    if state.team_in_possession == "Home":
        print("[AddFieldGoal] Update home period field goals")
        # Update home period field goals
        state.home_statistics.period_statistics[period_idx].scores.total = state.home_statistics.period_statistics[period_idx].scores.total + points
        state.home_statistics.period_statistics[period_idx].scores.field_goals = state.home_statistics.period_statistics[period_idx].scores.field_goals + 1

        execute_events(state)

        print("[AddFieldGoal] Update home total field goals")
        # Update home total field goals
        state.home_statistics.total_statistics.scores.total = state.home_statistics.total_statistics.scores.total + points
        state.home_statistics.total_statistics.scores.field_goals = state.home_statistics.total_statistics.scores.field_goals + 1

        execute_events(state)

    execute_events(state)

    print("[AddFieldGoal] Update away team statistics")
    # Update away team statistics
    if state.team_in_possession == "Away":
        print("[AddFieldGoal] Update away period field goals")
        # Update away period field goals
        state.away_statistics.period_statistics[period_idx].scores.field_goals = state.away_statistics.period_statistics[period_idx].scores.field_goals + 1
        state.away_statistics.period_statistics[period_idx].scores.total = state.away_statistics.period_statistics[period_idx].scores.total + points

        execute_events(state)

        print("[AddFieldGoal] Update away total field goals")
        # Update away total field goals
        state.away_statistics.total_statistics.scores.field_goals = state.away_statistics.total_statistics.scores.field_goals + 1
        state.away_statistics.total_statistics.scores.total = state.away_statistics.total_statistics.scores.total + points

        execute_events(state)

    execute_events(state)

    print("[AddFieldGoal] Update player statistics if enabled")
    # Update player statistics if enabled
    if state.include_players:
        print("[AddFieldGoal] Update home player statistics")
        # Update home player statistics
        if state.team_in_possession == "Home":
            print("[AddFieldGoal] Set variables")
            # Set variables
            state.home_player_selected_for_points_market.total_statistics.scores.total = state.home_player_selected_for_points_market.total_statistics.scores.total + points
            state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total + points

            execute_events(state)

        execute_events(state)

        print("[AddFieldGoal] Update away player statistics")
        # Update away player statistics
        if state.team_in_possession == "Away":
            print("[AddFieldGoal] Set variables")
            # Set variables
            state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total + points
            state.away_player_selected_for_points_market.total_statistics.scores.total = state.away_player_selected_for_points_market.total_statistics.scores.total + points
...
# From: step_add_penalty.py
# @depyler: custom_attribute_ignore = "step"
def add_penalty(state: State) -> None:
    print("[AddPenalty] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[AddPenalty] Add penalty points to home team")
    # Add penalty points to home team
    if state.team_in_possession == "Home":
        print("[AddPenalty] Set variables")
        # Set variables
    ...
    print("[AddPenalty] Add penalty points to away team")
    # Add penalty points to away team
    if state.team_in_possession == "Away":
        print("[AddPenalty] Set variables")
        # Set variables
    ...
    print("[AddPenalty] Update home team statistics")
    # Update home team statistics
    if state.team_in_possession == "Home":
        print("[AddPenalty] Update home total penalties")
        # Update home total penalties
        state.home_statistics.total_statistics.scores.penalties = state.home_statistics.total_statistics.scores.penalties + 1
        state.home_statistics.total_statistics.scores.total = state.home_statistics.total_statistics.scores.total + PENALTY_POINTS

        execute_events(state)

        print("[AddPenalty] Update home period penalties")
        # Update home period penalties
        state.home_statistics.period_statistics[period_idx].scores.total = state.home_statistics.period_statistics[period_idx].scores.total + PENALTY_POINTS
        state.home_statistics.period_statistics[period_idx].scores.penalties = state.home_statistics.period_statistics[period_idx].scores.penalties + 1

        execute_events(state)

    execute_events(state)

    print("[AddPenalty] Update away team statistics")
    # Update away team statistics
    if state.team_in_possession == "Away":
        print("[AddPenalty] Update away total penalties")
        # Update away total penalties
        state.away_statistics.total_statistics.scores.total = state.away_statistics.total_statistics.scores.total + PENALTY_POINTS
        state.away_statistics.total_statistics.scores.penalties = state.away_statistics.total_statistics.scores.penalties + 1

        execute_events(state)

        print("[AddPenalty] Update away period penalties")
        # Update away period penalties
        state.away_statistics.period_statistics[period_idx].scores.total = state.away_statistics.period_statistics[period_idx].scores.total + PENALTY_POINTS
        state.away_statistics.period_statistics[period_idx].scores.penalties = state.away_statistics.period_statistics[period_idx].scores.penalties + 1

        execute_events(state)

    execute_events(state)

    print("[AddPenalty] Update player statistics if enabled")
    # Update player statistics if enabled
    if state.include_players:
        print("[AddPenalty] Update home player statistics")
        # Update home player statistics
        if state.team_in_possession == "Home":
            print("[AddPenalty] Set variables")
            # Set variables
            state.home_player_selected_for_points_market.total_statistics.scores.conversions = state.home_player_selected_for_points_market.total_statistics.scores.conversions + 1
            state.home_player_selected_for_points_market.total_statistics.scores.total = state.home_player_selected_for_points_market.total_statistics.scores.total + PENALTY_POINTS
            state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.home_player_selected_for_points_market.period_statistics[period_idx].scores.total + PENALTY_POINTS
            state.home_player_selected_for_points_market.period_statistics[period_idx].scores.conversions = state.home_player_selected_for_points_market.period_statistics[period_idx].scores.conversions + 1
        ...
        print("[AddPenalty] Update away player statistics")
        # Update away player statistics
        if state.team_in_possession == "Away":
            print("[AddPenalty] Set variables")
            # Set variables
            state.away_player_selected_for_points_market.total_statistics.scores.total = state.away_player_selected_for_points_market.total_statistics.scores.total + PENALTY_POINTS
            state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total = state.away_player_selected_for_points_market.period_statistics[period_idx].scores.total + PENALTY_POINTS
            state.away_player_selected_for_points_market.period_statistics[period_idx].scores.conversions = state.away_player_selected_for_points_market.period_statistics[period_idx].scores.conversions + 1
            state.away_player_selected_for_points_market.total_statistics.scores.conversions = state.away_player_selected_for_points_market.total_statistics.scores.conversions + 1
...
# From: step_add_try.py
# @depyler: custom_attribute_ignore = "step"
def add_try(state: State) -> None:
    print("[AddTry] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[AddTry] Add try points to home team")
    # Add try points to home team
    if state.team_in_possession == "Home":
        print("[AddTry] Update home match score")
        # Update home match score
        state.home_match_score = state.home_match_score + TRY_POINTS

        execute_events(state)

        print("[AddTry] Update home period statistics")
        # Update home period statistics
        state.home_statistics.period_statistics[period_idx].scores.tries = state.home_statistics.period_statistics[period_idx].scores.tries + 1
        state.home_statistics.period_statistics[period_idx].scores.total = state.home_statistics.period_statistics[period_idx].scores.total + TRY_POINTS

        execute_events(state)

        print("[AddTry] Update home total statistics")
        # Update home total statistics
        state.home_statistics.total_statistics.scores.tries = state.home_statistics.total_statistics.scores.tries + 1
        state.home_statistics.total_statistics.scores.total = state.home_statistics.total_statistics.scores.total + TRY_POINTS

        execute_events(state)

    execute_events(state)

    print("[AddTry] Add try points to away team")
    # Add try points to away team
    if state.team_in_possession == "Away":
        print("[AddTry] Update away match score")
        # Update away match score
        state.away_match_score = state.away_match_score + TRY_POINTS

        execute_events(state)

        print("[AddTry] Update away period statistics")
        # Update away period statistics
        state.away_statistics.period_statistics[period_idx].scores.total = state.away_statistics.period_statistics[period_idx].scores.total + TRY_POINTS
        state.away_statistics.period_statistics[period_idx].scores.tries = state.away_statistics.period_statistics[period_idx].scores.tries + 1

        execute_events(state)

        print("[AddTry] Update away total statistics")
        # Update away total statistics
        state.away_statistics.total_statistics.scores.tries = state.away_statistics.total_statistics.scores.tries + 1
        state.away_statistics.total_statistics.scores.total = state.away_statistics.total_statistics.scores.total + TRY_POINTS
...
# From: step_assign_try.py
# @depyler: custom_attribute_ignore = "step"
def assign_try(state: State, player_index: int, players: List[Player], team: str) -> None:
    print("[AssignTry] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[AssignTry] Get players list based on team")
    # Get players list based on team
    players = (state.home_players if team == "Home" else state.away_players)

    execute_events(state)

    print("[AssignTry] Find player by index")
    # Find player by index
    player = (next((p for p in players if p.player_index == player_index), None))
    player_list_idx = (next((i for i, p in enumerate(players) if p.player_index == player_index), -1))

    execute_events(state)

    print("[AssignTry] Update period try scorers")
    # Update period try scorers
    state.total_try_scorers = state.total_try_scorers + [player_list_idx]
    state.period_try_scorers[period_idx] = state.period_try_scorers[period_idx] + [player_list_idx]

    execute_events(state)

    print("[AssignTry] Update home team try scorers")
    # Update home team try scorers
    if team == "Home":
        print("[AssignTry] Set variables")
        # Set variables
        state.home_total_try_scorers = state.home_total_try_scorers + [player_list_idx]
        state.home_period_try_scorers[period_idx] = state.home_period_try_scorers[period_idx] + [player_list_idx]

        execute_events(state)

    execute_events(state)

    print("[AssignTry] Update away team try scorers")
    # Update away team try scorers
    if team == "Away":
        print("[AssignTry] Set variables")
        # Set variables
        state.away_total_try_scorers = state.away_total_try_scorers + [player_list_idx]
        state.away_period_try_scorers[period_idx] = state.away_period_try_scorers[period_idx] + [player_list_idx]

        execute_events(state)

    execute_events(state)

    print("[AssignTry] Update player period statistics")
    # Update player period statistics
    player.period_statistics[period_idx].scores.tries = player.period_statistics[period_idx].scores.tries + 1
    player.period_statistics[period_idx].scores.total = player.period_statistics[period_idx].scores.total + TRY_POINTS

    execute_events(state)

    print("[AssignTry] Update player total statistics")
    # Update player total statistics
    ...
    print("[AssignTry] Get team statistics")
    # Get team statistics
    team_stats = (state.home_statistics if team == "Home" else state.away_statistics)

    execute_events(state)

    print("[AssignTry] Update team player statistics")
    # Update team player statistics
    team_stats.player_statistics[player_list_idx].scores.tries = team_stats.player_statistics[player_list_idx].scores.tries + 1
    team_stats.player_statistics[player_list_idx].scores.total = team_stats.player_statistics[player_list_idx].scores.total + TRY_POINTS
...
def conversion(state: State) -> None:
    ...
    conversion_result = get_conversion_model_result(state)
...
def field_goal(state: State) -> None:
    ...
    field_goal_result = get_field_goal_model_result(state)
...
def get_kickoff_model_result(state: State) -> GetKickoffModelResultOutputs:
    ...
    return GetKickoffModelResultOutputs(
        ...
        grid_result=KICKOFF_TO_GRID_RESULT[str(kickoff_result)]
    )
...
def next_play(state: State) -> NextPlayOutputs:
    ...
    # Run next play model
    model: NextPlayInferOutputs0 = infer[NextPlayInferOutputs0](name="next_play", input=[float(state.tackles), float(distance_to_try_line), float(calculate_dist_from_centre(state)), float(get_team_handicap(state)), float(state.simulation_invariants.total_points), float(state.home_match_score + state.away_match_score), float(max(min(possessing_team_margin / 2.0, 10.0), -10.0)), btf(player_advantage == 1), btf(player_advantage > 1), btf(player_advantage == -1), btf(player_advantage < -1), float(0.8 if 0 < distance_to_try_line <= 10 else (0.06 if 10 < distance_to_try_line <= 20 else 0.0))])
...
def player_meters(state: State) -> PlayerMetersOutputs:
    ...
    result = infer(name="player_meters", input=[])
...
def process_tackle(state: State) -> None:
    ...
    print("[ProcessTackle] Get period index")
    # Get period index
    period_idx = state.period.number - 1

    execute_events(state)

    print("[ProcessTackle] Get team statistics")
    # Get team statistics
    team_stats = (state.home_statistics if state.team_in_possession == "Home" else state.away_statistics)

    execute_events(state)

    print("[ProcessTackle] Update period tackles")
    # Update period tackles
    team_stats.period_statistics[period_idx].tackles = team_stats.period_statistics[period_idx].tackles + 1

    execute_events(state)

    print("[ProcessTackle] Update total tackles")
    # Update total tackles
...
def xy(state: State) -> XyOutputs:
    ...
    # Get grid result
    grid_result = GRID_RESULT[int(sample(model.probabilities, random.random()))]
    ...
    return XyOutputs(
        field_position=convert_field_position(state, grid_result, state.team_in_possession)
    )
...
def execute_events(state: State) -> None:
    ...
    if False and 'period_first_last_score_helper' not in state.executed_events:
        period_first_last_score_helper(state)
        state.executed_events.add('period_first_last_score_helper')
    if state.game_status in ('HalfTime', 'Period2', 'AwaitingExtraTime', 'ExtraTime', 'Ended') or state.is_over and 'first_half_output_event' not in state.executed_events:
        first_half_output_event(state)
        state.executed_events.add('first_half_output_event')
    if state.game_status in ('AwaitingExtraTime', 'ExtraTime', 'Ended') and 'normal_time_output_event' not in state.executed_events:
        normal_time_output_event(state)
        state.executed_events.add('normal_time_output_event')
    if state.include_players:
        player_score_tracking_event(state)
    if len(state.incidents) > 0 or state.is_over and 'minute_winner_tracking_event' not in state.executed_events:
        minute_winner_tracking_event(state)
        state.executed_events.add('minute_winner_tracking_event')
    if state.is_over and 'debug' not in state.executed_events:
        debug(state)
        state.executed_events.add('debug')
    if state.include_players and 'player_void_output_event' not in state.executed_events:
        player_void_output_event(state)
        state.executed_events.add('player_void_output_event')
    if state.game_status == 'Ended' or state.is_over and 'end_of_game_output_event' not in state.executed_events:
        end_of_game_output_event(state)
        state.executed_events.add('end_of_game_output_event')
    if state.home_match_score > 0 and 'test_once_event' not in state.executed_events:
        test_once_event(state)
        state.executed_events.add('test_once_event')
    if True:
        update_game_status(state)
    if len(state.incidents) > 0 or state.is_over and 'first_to_score_tracking_event' not in state.executed_events:
        first_to_score_tracking_event(state)
        state.executed_events.add('first_to_score_tracking_event')
    if len(state.incidents) > 0 or state.is_over and 'race_to_points_tracking_event' not in state.executed_events:
        race_to_points_tracking_event(state)
        state.executed_events.add('race_to_points_tracking_event')
    if state.game_status not in ('UnknownStatus', 'NotStarted') and 'anytime_output_event' not in state.executed_events:
